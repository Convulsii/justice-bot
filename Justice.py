...
# ========== ФУНКЦИЯ ОБНОВЛЕНИЯ ГОЛОСОВОГО ВРЕМЕНИ ==========
@bot.event
async def on_voice_state_update(member, before, after):
    # Обновление времени в голосовом канале
    if before.channel is None and after.channel is not None:
        # Зашёл в голосовой канал
        async with aiosqlite.connect("justice.db") as db:
            await db.execute('UPDATE users SET voice_join_time=? WHERE user_id=? AND guild_id=?', (datetime.now().isoformat(), member.id, member.guild.id))
            await db.commit()
    elif before.channel is not None and after.channel is None:
        # Вышел из голосового канала
        async with aiosqlite.connect("justice.db") as db:
            cur = await db.execute('SELECT voice_join_time, voice_total_seconds FROM users WHERE user_id=? AND guild_id=?', (member.id, member.guild.id))
            row = await cur.fetchone()
            if row and row[0]:
                join_time = datetime.fromisoformat(row[0])
                seconds = int((datetime.now() - join_time).total_seconds())
                total = (row[1] or 0) + seconds
                await db.execute('UPDATE users SET voice_total_seconds=?, voice_join_time=? WHERE user_id=? AND guild_id=?', (total, None, member.id, member.guild.id))
                await db.commit()
                # Обновление недельной статистики
                week_start = datetime.now().strftime("%Y-%m-%d")
                cur2 = await db.execute('SELECT voice_minutes FROM weekly_stats WHERE user_id=? AND guild_id=? AND week_start=?', (member.id, member.guild.id, week_start))
                week_row = await cur2.fetchone()
                if week_row:
                    new_minutes = week_row[0] + (seconds // 60)
                    await db.execute('UPDATE weekly_stats SET voice_minutes=? WHERE user_id=? AND guild_id=? AND week_start=?', (new_minutes, member.id, member.guild.id, week_start))
                else:
                    await db.execute('INSERT INTO weekly_stats (user_id, guild_id, week_start, voice_minutes) VALUES (?,?,?,?)', (member.id, member.guild.id, week_start, seconds//60))
                await db.commit()
...
async def warn(ctx, member: discord.Member, days: int = 7, *, reason: str = "Не указана"):
    ...
    async with aiosqlite.connect("justice.db") as db:
        ...
        count = (await cur.fetchone())[0]
        await db.commit()
    embed = discord.Embed(title="⚠️ ПРЕДУПРЕЖДЕНИЕ", color=discord.Color.orange(), timestamp=datetime.now())
    ...
    await ctx.send(embed=embed)
    await log_action(ctx.guild.id, "⚠️ ВЫДАНО ПРЕДУПРЕЖДЕНИЕ", f"{ctx.author.mention} выдал предупреждение {member.mention}\nПричина: {reason}")
    try:
...
async def warns(ctx, member: discord.Member = None):
    ...
    if not warnings:
        return await ctx.send(f"📭 У {target.mention} нет активных предупреждений")
    ...
    for wid, mod_id, reason, expires in warnings[:10]:
        ...
        embed.add_field(name=f"#{wid}", value=f"Модератор: {mod_name}\nПричина: {reason}\nИстекает: <t:{int(datetime.fromisoformat(expires).timestamp())}:R>", inline=False)
    if len(warnings) > 10:
...
async def unwarn(ctx, member: discord.Member, warn_id: int):
    async with aiosqlite.connect("justice.db") as db:
        ...
        if not await cur.fetchone():
            return await ctx.send(f"❌ Предупреждение #{warn_id} не найдено у {member.mention}")
        await db.execute('DELETE FROM warnings WHERE id=?', (warn_id,))
        await db.commit()
    await ctx.send(f"✅ Снято предупреждение #{warn_id} у {member.mention}")
    await log_action(ctx.guild.id, "🔓 СНЯТО ПРЕДУПРЕЖДЕНИЕ", f"{ctx.author.mention} снял предупреждение #{warn_id} у {member.mention}")
...
async def mute(ctx, member: discord.Member, duration: str, *, reason: str = "Не указана"):
    ...
    for char in duration:
        if char.isdigit():
            ...
            num = ""
    if seconds <= 0:
        return await ctx.send("❌ Пример: `j.mute @user 10м` или `j.mute @user 1ч`")
    until = datetime.now() + timedelta(seconds=seconds)
    await member.timeout(until, reason=reason)
    embed = discord.Embed(title="🔇 МУТ", color=discord.Color.red(), timestamp=datetime.now())
    embed.add_field(name="👤 Пользователь", value=member.mention, inline=True)
    embed.add_field(name="🛡️ Модератор", value=ctx.author.mention, inline=True)
    ...
    await ctx.send(embed=embed)
    await log_action(ctx.guild.id, "🔇 МУТ", f"{ctx.author.mention} замутил {member.mention} на {duration}\nПричина: {reason}")
    try:
...
async def unmute(ctx, member: discord.Member, *, reason: str = "Не указана"):
    ...
    embed.add_field(name="🛡️ Модератор", value=ctx.author.mention, inline=True)
    await ctx.send(embed=embed)
    await log_action(ctx.guild.id, "🔊 СНЯТ МУТ", f"{ctx.author.mention} снял мут с {member.mention}")
...
async def ban(ctx, member: discord.Member, duration: str = None, *, reason: str = "Не указана"):
    if duration:
        ...
        await member.ban(reason=reason)
        await ctx.send(f"🔨 {member.mention} забанен навсегда")
    await log_action(ctx.guild.id, "🔨 БАН", f"{ctx.author.mention} забанил {member.mention}\nПричина: {reason}")
...
async def kick(ctx, member: discord.Member, *, reason: str = "Не указана"):
    ...
    embed.add_field(name="📝 Причина", value=reason, inline=True)
    await ctx.send(embed=embed)
    await log_action(ctx.guild.id, "👢 КИК", f"{ctx.author.mention} кикнул {member.mention}\nПричина: {reason}")
...
async def clear(ctx, amount: int = 10):
    ...
    await msg.delete()
    await log_action(ctx.guild.id, "🗑️ ОЧИСТКА", f"{ctx.author.mention} очистил {len(deleted)-1} сообщений в {ctx.channel.mention}")
...
async def nickname(ctx, member: discord.Member, *, nickname: str = None):
    ...
    if nickname:
        ...
        await log_action(ctx.guild.id, "✏️ СМЕНА НИКА", f"{ctx.author.mention} изменил ник {member.mention} на {nickname}")
    else:
        await ctx.send(f"✅ Никнейм {old_name} сброшен")
        await log_action(ctx.guild.id, "✏️ СБРОС НИКА", f"{ctx.author.mention} сбросил ник {member.mention}")

# ========== КОМАНДЫ ДЛЯ ВЛАДЕЛЬЦА (ПРОДОЛЖЕНИЕ) ==========
@bot.command()
@commands.check(is_owner)
...
# ========== ФУНКЦИЯ ОБНОВЛЕНИЯ НЕДЕЛЬНОЙ СТАТИСТИКИ ==========
    ...
    for inv in invites:
        if inv.uses > 0:
            async with aiosqlite.connect("justice.db") as db:
                ...
                await db.commit()
                await update_user(inv.inviter.id, member.guild.id, total_invites=inv.inviter.id)
            break
    # Выдача стандартной роли
    role = member.guild.get_role(DEFAULT_ROLE_ID)
    if role:
        try: await member.add_roles(role)
        except: pass
...
# ========== ВИСЕЛИЦА (ПРОДОЛЖЕНИЕ) ==========
hangman_words = ["ПИТОН", "ДИСКОРД", "БОТ", "СЕРВЕР", "ПРОГРАММИРОВАНИЕ", "РАЗРАБОТЧИК", "КОМАНДА", "ИГРА", "ПОБЕДА", "УДАЧА", "ЭКОНОМИКА", "ФЕРМА", "РЫБАЛКА", "МАГАЗИН", "ТИКЕТЫ", "МОДЕРАЦИЯ", "АВТОМОД", "ИНВЕСТИЦИИ", "КРАФТ", "ЖИВОТНЫЕ"]
hangman_games = {}
...
async def hangman(ctx):
    if ctx.channel.id in hangman_games:
        return await ctx.send("❌ Игра уже идёт в этом канале!")
    word = random.choice(hangman_words)
    game = HangmanGame(word)
    hangman_games[ctx.channel.id] = game
...
async def guess(ctx, letter: str = None):
    if ctx.channel.id not in hangman_games:
        return await ctx.send("❌ Нет активной игры! Начните: `j.hangman`")
...
async def short(ctx, *, url: str = None):
    if not url:
        return await ctx.send("❌ Введите ссылку: `j.short https://example.com`")
...
async def craft(ctx, recipe_id: str = None):
    if not recipe_id or recipe_id.lower() not in RECIPES:
        recipes_list = "\n".join([f"• {rid} - {data['name']} | {data['desc']}" for rid, data in RECIPES.items()])
        return await ctx.send(f"🔨 **Доступные рецепты**\n{recipes_list}\n\nПример: `j.craft золотой слиток`")
    recipe = RECIPES[recipe_id.lower()]
    async with aiosqlite.connect("justice.db") as db:
        cur = await db.execute('SELECT inventory FROM users WHERE user_id=? AND guild_id=?', (ctx.author.id, ctx.guild.id))
        inv = json.loads((await cur.fetchone())[0] or "[]")
        ...
        if missing:
            return await ctx.send(f"❌ Не хватает ингредиентов:\n" + "\n".join(missing))
        for ing, amount in recipe["ingredients"].items():
            for _ in range(amount):
        ...
        await add_xp(ctx.author.id, ctx.guild.id, recipe["xp"])
        await db.commit()
    await ctx.send(f"✅ Вы скрафтили {recipe['name']} x{recipe['count']}!\n✨ +{recipe['xp']} XP")
...
# ========== УЛУЧШЕНИЯ ФЕРМЫ (ПРОДОЛЖЕНИЕ) ==========
@bot.command()
async def farm_upgrades_info(ctx):
    """📈 Показать информацию об улучшениях фермы"""
    async with aiosqlite.connect("justice.db") as db:
...
# ========== БОНУС ЗА ПРИГЛАШЕНИЯ (ПОДРОБНЕЕ) ==========
@bot.command()
...
# ========== ЕЖЕДНЕВНЫЕ ЗАДАНИЯ (СБРОС) ==========
@tasks.loop(hours=24)
async def daily_quests_reset():
    """Сброс ежедневных заданий каждый день в 00:00"""
    await asyncio.sleep(random.randint(0, 3600))
    async with aiosqlite.connect("justice.db") as db:
        await db.execute('DELETE FROM daily_quests WHERE quest_date < date("now")')
        await db.commit()
    print("✅ Ежедневные задания сброшены")

# ========== АКТИВНЫЕ БУСТЕРЫ (ПРОВЕРКА) ==========
@tasks.loop(minutes=5)
async def check_boosters():
    """Проверка и удаление просроченных бустеров"""
    now = time.time()
    to_remove = []
    for user_id, boosters in active_boosters.items():
        for booster_type, data in list(boosters.items()):
            if data["end"] < now:
                to_remove.append((user_id, booster_type))
    for user_id, booster_type in to_remove:
        if user_id in active_boosters and booster_type in active_boosters[user_id]:
            del active_boosters[user_id][booster_type]
            user = bot.get_user(user_id)
            if user:
                await user.send(f"⏰ Бустер **{booster_type.upper()}** закончился!")

# ========== ЕЖЕНЕДЕЛЬНЫЙ БОНУС ==========
@bot.command()
async def weekly_bonus(ctx):
    """🎁 Еженедельный бонус (дополнительный)"""
    data = await get_user(ctx.author.id, ctx.guild.id)
...
async def monthly_bonus(ctx):
    """🎁 Ежемесячный бонус (дополнительный)"""
    data = await get_user(ctx.author.id, ctx.guild.id)
...
async def hourly_bonus(ctx):
    """🎁 Бонус каждый час"""
    ready, remaining = check_cooldown(ctx.author.id, "hourly")
    if not ready:
...
# Добавляем часовой бонус в GAME_COOLDOWN
GAME_COOLDOWN["hourly"] = 3600
...
async def top_weekly_voice(ctx):
    """🏆 Топ по голосовому онлайну за неделю"""
    week_start = (datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d")
    async with aiosqlite.connect("justice.db") as db:
...
async def top_weekly_casino(ctx):
    """🏆 Топ по выигрышам в казино за неделю"""
    week_start = (datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d")
    async with aiosqlite.connect("justice.db") as db:
...
async def stats(ctx, member: discord.Member = None):
    ...
    embed.set_thumbnail(url=target.display_avatar.url)
    # Базовая статистика
    embed.add_field(name="💬 Сообщений", value=data[9] if len(data)>9 else 0, inline=True)
    embed.add_field(name="📅 Сегодня", value=data[21] if len(data)>21 else 0, inline=True)
    embed.add_field(name="📅 Неделя", value=data[22] if len(data)>22 else 0, inline=True)
    embed.add_field(name="📅 Месяц", value=data[23] if len(data)>23 else 0, inline=True)
    # Игровая статистика
    embed.add_field(name="🎰 Побед в казино", value=data[33] if len(data)>33 else 0, inline=True)
    embed.add_field(name="🃏 Побед в блэкджек", value=data[34] if len(data)>34 else 0, inline=True)
    embed.add_field(name="❌⭕ Побед в крестики", value=data[35] if len(data)>35 else 0, inline=True)
    # Ферма
    embed.add_field(name="🌱 Посажено", value=data[40] if len(data)>40 else 0, inline=True)
    embed.add_field(name="🌾 Собрано", value=data[39] if len(data)>39 else 0, inline=True)
    # Рыбалка
    embed.add_field(name="🎣 Поймано рыб", value=data[36] if len(data)>36 else 0, inline=True)
    embed.add_field(name="🐉 Легендарных", value=data[37] if len(data)>37 else 0, inline=True)
    embed.add_field(name="✨ Мифических", value=data[38] if len(data)>38 else 0, inline=True)
    # Работа и ограбления
    embed.add_field(name="💼 Работ", value=data[41] if len(data)>41 else 0, inline=True)
    embed.add_field(name="🔫 Успешных ограблений", value=data[42] if len(data)>42 else 0, inline=True)
    # Приглашения
    embed.add_field(name="📨 Приглашений", value=data[25] if len(data)>25 else 0, inline=True)
    # Голосовой онлайн
    voice_sec = data[32] if len(data)>32 else 0
    voice_hours = voice_sec // 3600
    voice_min = (voice_sec % 3600) // 60
    embed.add_field(name="🎤 В голосовых каналах", value=f"{voice_hours}ч {voice_min}мин", inline=True)
    await ctx.send(embed=embed)
...
async def achievements(ctx, member: discord.Member = None):
    ...
    if not earned:
        ...
        text = ""
        for ach_id, date in earned[:20]:
            ach_data = ACHIEVEMENTS.get(ach_id, {"name": ach_id})
            text += f"✅ {ach_data['name']} – <t:{int(datetime.fromisoformat(date).timestamp())}:R>\n"
        embed.description = text
        if len(earned) > 20:
            embed.set_footer(text=f"Всего достижений: {len(earned)}")
    await ctx.send(embed=embed)

# ========== ОБНОВЛЕНИЕ СТАТИСТИКИ ПРИ ДЕЙСТВИЯХ ==========
...
if __name__ == "__main__":
    print(":rocket: Запуск Justice Bot...")
    if TOKEN == "ВСТАВЬ_СВОЙ_ТОКЕН":
        print(":x: ВСТАВЬ ТОКЕН!")
        exit(1)
    bot.run(TOKEN)
